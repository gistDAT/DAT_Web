...
def home_view(request):
    ...
    if request.method == "POST":
        ...
        search.send_keys(real_Keyword[0])
        time.sleep(1)

        search.send_keys(',')
        time.sleep(1)

        search.send_keys(real_Keyword[1])
        time.sleep(1)

        search.send_keys(',')
        time.sleep(1)

        search.send_keys(real_Keyword[2])
        time.sleep(1)
        search.send_keys(',')
        time.sleep(1)
        ...
        title = driver.find_element_by_xpath('//input[@id="thumbnail"]')
        link = title.get_attribute("href")
        out_put = driver.get(link)
        return render(request,'subapp/sub_view.html', context={'output':out_put},)
    else:
